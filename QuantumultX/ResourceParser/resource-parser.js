    ...
    if (server.indexOf('@') != -1) {
      ...
      var ipport =
        server.split('#')[0].indexOf(':') == -1
          ? 'http=' + Base64.decode(server.split('#')[0])
          : 'http=' + server.split('#')[0]; // https://b64(ipport)
    }
...
function VR2QX(subs, Pudp, Ptfo, Pcert0, PTls13) {
  ...
  if (obfs == 'none') {
    ...
    $notify(
      '⚠️ Quantumult X 暂不支持 grpc 类型 vmess节点，已忽略此条',
      '',
      subs
    );
    pdrop = 1;
  ...
  for (var i = 0; i < cnt.length; i++) {
    if (cnt[i]) {
      ...
      if (
        ...
        cnti =
          cnti.replace(/ /g, '').split('data=')[0] + ' url ' + 'reject-dict';
...
function YAMLFix(cnt) {
  ...
  //2022-11-29 修改
  cnt =
    cnt.indexOf('proxies:') != -1 && cnt.indexOf('\ns{4}server') != -1
      ? cnt
          .replace(/\n\s{4}headers/g, '\n      headers')
          .replace(/\n\s{6}Host/g, '\n        Host')
          .replace(/\t/g, '')
